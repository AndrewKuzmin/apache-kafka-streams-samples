...
import com.phylosoft.learning.kafka.streams.kstream.interactivequeries.common.BaseInteractiveQueriesRestService;
 ...
 */
@Path("state")
public class WordCountInteractiveQueriesRestService extends BaseInteractiveQueriesRestService {

    WordCountInteractiveQueriesRestService(final KafkaStreams streams,
                                           final HostInfo hostInfo) {
        super(streams, hostInfo);
    }
    ...
    public KeyValueBean byKey(@PathParam("storeName") final String storeName,
        ...
        final ReadOnlyKeyValueStore<String, Long> store = getStreams().store(storeName, QueryableStoreTypes.<String, Long>keyValueStore());
        if (store == null) {
    ...
    private KeyValueBean fetchByKey(final HostStoreInfo host, final String path) {
        return getClient().target(String.format("http://%s:%d/%s", host.getHost(), host.getPort(), path))
    ...
    public List<KeyValueBean> windowedByKey(@PathParam("storeName") final String storeName,
        ...
        final ReadOnlyWindowStore<String, Long> store = getStreams().store(storeName,
    ...
    private List<KeyValueBean> rangeForKeyValueStore(final String storeName,
        ...
        final ReadOnlyKeyValueStore<String, Long> store = getStreams().store(storeName, QueryableStoreTypes.keyValueStore());
